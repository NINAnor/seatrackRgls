...
#'
#' @export
twilightCalc_bugfree <- function(datetime, light, LightThreshold = TRUE, preSelection = TRUE, maxLight = NULL, ask = TRUE, nsee = 500, allTwilights = FALSE) {
  if (class(datetime)[1] != "POSIXct") {
    stop(sprintf("datetime need to be provided as POSIXct class object."), call. = F)
  } else {
    bas <- data.frame(datetime = as.POSIXct(datetime, "UTC"), light)
  }
  ...
  out <- i.preSelection_bugfree(bas$datetime, bas$light, LightThreshold)[, -1]
  ...
  if (ask) {
    ...
    for (i in 1:nn) {
      ...
      repeat{
        plot(bas[sub, 1], bas[sub, 2],
          type = "o", cex = 0.6, pch = 20, ylab = "Light intensity", xaxs = "i", xaxt = "n", xlab = "",
        ...
        axis(1,
          at = out[seq(from = 1, to = nrow(out), length.out = (nrow(out) %/% 2)), 1],
  ...
  raw <- data.frame(
    datetime = c(as.POSIXct(datetime, "UTC"), as.POSIXct(out$datetime, "UTC")),
