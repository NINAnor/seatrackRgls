#' Estimate and Filter Winter Locations from GLS Data
#'
...
#' @return A data frame with refined winter locations added to the main manually calibrated track.
#' @export
winter_locations <- function(df, luxfile, speed, boundary.box, loess_filter_k, months_breeding, species,
  ...
  light_threshold <- (min(as.numeric(lu$lux)) + 0.1)
  # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  if (is.null(sun_angle_start)) {
    sun_angle_start <- as.numeric(-5)
  }
  if (is.na(sun_angle_start)) {
    temp$sun <- as.numeric(-5)
  }
  if (!is.na(sun_angle_start)) {
  ...
  add_to_winter <- posdata[yday(posdata$date_time) %in% winter_days, ]
  add_to_winter <- add_to_winter[!(as.Date(add_to_winter$date_time) %in% as.Date(normal_threshold_df$date_time)), ]
  add_to_winter <- add_to_winter[order(add_to_winter$date_time), ]
  output <- add_to_winter
  return(output)
}
